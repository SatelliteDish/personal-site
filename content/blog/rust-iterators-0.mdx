...
`take` is really simple but deceptively useful, it takes a `usize` and returns at most that many elements from the front. If there are less than the requested items left it just returns what is left. This is incredibly useful when working with data that is large enough that you don't want it all in memory at once, as you can `.take` one chunk at a time. It gracefully handles asking for too much, so you don't need to know the size beforehand.
...
Without `take` the first example it would run forever because the iterator has no end. We'll go over infinite iterators a little more soon.
...
This works fine, but calling something like `sum` or `collect` on `squares` would cause this to loop infinitely. Iterators can also be of indeterminate size, for example a list input for a CLI app could be implemented as an iterator. Each time `next` is called it prompts the user to enter another entry, and on a specific input it returns `None` and finishes iteration. When the user will stop is never known by anyone but the user, but because iterators are lazy this doesn't matter.
